...
touch /vault/secrets/init-output.txt
...
if ! vault status | grep -q "Initialized.*true"; then
    ...
    vault operator init -key-shares=5 -key-threshold=3 > /vault/secrets/init-output.txt
    echo "Vault initialized. Keys stored in /vault/secrets/init-output.txt"

    # Unseal Vault
    for i in 1 2 3; do
        vault operator unseal $(grep "Unseal Key $i:" /vault/secrets/init-output.txt | awk '{print $4}')
    done

    # Login with root token
    VAULT_ROOT_TOKEN=$(grep "Initial Root Token:" /vault/secrets/init-output.txt | awk '{print $4}')
    export VAULT_ROOT_TOKEN
    echo -e "\nVAULT_ROOT_TOKEN=$VAULT_ROOT_TOKEN" >> /secrets/.env
