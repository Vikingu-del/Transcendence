...
VAULT_RESPONSE=$(curl -s --header "X-Vault-Token: $VAULT_TOKEN" \
    $VAULT_ADDR/v1/secret/data/game_db)

# Get application credentials from Vault
APP_USER=$(echo $VAULT_RESPONSE | jq -r .data.data.GAME_DB_USER)
APP_PASSWORD=$(echo $VAULT_RESPONSE | jq -r .data.data.GAME_DB_PASSWORD)
APP_DB=$(echo $VAULT_RESPONSE | jq -r .data.data.GAME_DB_NAME)
