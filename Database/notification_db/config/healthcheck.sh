...
if [ ! -f "$ROLE_ID_FILE" ] || [ ! -f "$SECRET_ID_FILE" ]; then
    ...
    ls -la /vault/approle/notification_db >> /var/log/postgresql/healthcheck.log
    exit 1
fi
...
DB_USER=$(curl -s --header "X-Vault-Token: $VAULT_TOKEN" \
    $VAULT_ADDR/v1/secret/data/notification_db | jq -r .data.data.NOTIFICATION_DB_USER)

DB_NAME=$(curl -s --header "X-Vault-Token: $VAULT_TOKEN" \
    $VAULT_ADDR/v1/secret/data/notification_db | jq -r .data.data.NOTIFICATION_DB_NAME)
